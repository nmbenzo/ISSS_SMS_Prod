...
def find_isss_advisor():
    ...
    advisor list
    ::returns a string of the advisor's name
    """
    ...
    for i in major_advisor.Nina:
        if i.lower() in INBOUND_MESSAGE \
            and ('bachelor' in INBOUND_MESSAGE):
            return 'Nina'
...
def send_sms_response():
    ...
    if ('appointment' in INBOUND_MESSAGE) or ('advisor' in INBOUND_MESSAGE):
        ...
        and ('credit' in INBOUND_MESSAGE) or ('full time' in INBOUND_MESSAGE)\
        or ('credit' in INBOUND_MESSAGE) or ('unit' in INBOUND_MESSAGE):
        ...
        or ('extern' in INBOUND_MESSAGE) or ('employment' in INBOUND_MESSAGE):
        resp.message(menu_resp['employment'])
        logger.info(f'{resp}')
        return str(resp)
    elif ('ssn' in INBOUND_MESSAGE) or ('social security' in INBOUND_MESSAGE) \
